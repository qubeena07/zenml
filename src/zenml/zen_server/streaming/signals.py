...
class StreamEndEventHandler(EventHandler):
    ...
    @staticmethod
    async def create() -> "StreamEndEventHandler":
        """Unused factory; the streaming subsystem constructs the handler directly with its deps."""
        raise NotImplementedError(
            "StreamEndEventHandler is constructed by `initialize_streaming` "
            "with explicit dependencies, not via the source-loading flow."
        )
