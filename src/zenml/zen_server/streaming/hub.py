...
class StreamHub:
    ...
    def __init__(
        ...
        *,
        ...
        """Wire up the hub around an `EventBroker`."""
        self._broker = broker
    ...
    async def attach(
        ...
        *,
        terminate_after_catchup: bool = False,
    ) -> AsyncGenerator[YieldItem, None]:
        """Subscribe to a stream as a consumer.

        If `terminate_after_catchup` is True, an `EndMarker` is yielded
        after the catch-up phase completes (unless the catch-up itself
        already delivered the terminal event). The marker is scoped to
        this consumer — other consumers on the same session are
        unaffected.
        """
    ...
    async def shutdown(self) -> None:
        """Hub shutdown: broadcast a gap, then cancel reader + grace tasks.

        Consumers don't actually unwind here — they're still parked on
        `queue.get()` waiting for the next item. Termination happens
        when the surrounding HTTP request task is cancelled by the
        server (uvicorn does this at app shutdown). The gap broadcast
        is best-effort: clients see one last `gap: shutdown` SSE frame
    ...
    async def _reader_loop(self, session: _Session) -> None:
        ...
        while not session.closed.is_set():
            try:
                ...
                # events the live consumers already saw. If we can't
                # resolve a head after retries, abandon the session.
                try:
