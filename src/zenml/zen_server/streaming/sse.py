...
# SSE response headers — disable caching everywhere in the path and
# turn off response buffering on nginx-style intermediaries.
...
# Per-event byte cap on the wire. Producers blasting megabyte payloads
# would otherwise fill the broker's MAXLEN budget and starve other
# tenants.
EVENT_PAYLOAD_BYTES_MAX: int = 64 * 1024
...
async def sse_stream(
    *,
    ...
    agen = hub.attach(
        stream_key,
        from_id=from_id,
        terminate_after_catchup=terminate_after_catchup,
    )
    ...
    try:
        while True:
            ...
            framed = _frame_for(item, kind_filter, run_id)
            if framed is None:
                continue
...
def _frame_for(
    ...
    if isinstance(frame, UnknownFrame):
        ...
        # understand. Emit a `ping` event so the client's Last-Event-ID
        # advances past it.
        return _filtered_ping(item.id), False

    event = frame.event
    if kind_filter is not None and event.kind not in kind_filter:
        # Filtered out — emit a `ping` event carrying the id so the
...
def _filtered_ping(event_id: str) -> bytes:
    """SSE frame that advances Last-Event-ID without surfacing data."""
    return format_sse_frame("ping", "{}", event_id=event_id)
