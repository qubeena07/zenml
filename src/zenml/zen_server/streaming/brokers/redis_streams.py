...
class RedisStreamsBroker(EventBroker):
    """Broadcast event broker backed by Redis Streams."""
    ...
    async def read(
        ...
        *,
        ...
        # Truncation detection only when XREAD returned nothing: if it
        # returned messages, the cursor wasn't past the oldest retained
        # id at read time. Skipping the XRANGE on the hot path halves
        # the RTT count of the reader loop.
        if from_id is not None and not messages:
            try:
