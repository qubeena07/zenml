...
from zenml.streams import publisher as publisher_module
...
class _FakeZenStore:
    ...
    def publish_run_events(
        self, pipeline_run_id, batch: EventBatchRequest
    ) -> None:
...
) -> publisher_module._StreamPublisher:
    """Fresh publisher; shut down at test teardown."""
    monkeypatch.setattr(publisher_module, "_publisher", None)
    instance = publisher_module._StreamPublisher()
    yield instance
...
def test_publish_delivers_single_event(
    publisher: publisher_module._StreamPublisher, fake_store: _FakeZenStore
):
...
def test_drops_when_zen_store_unavailable(
    publisher: publisher_module._StreamPublisher,
    monkeypatch: pytest.MonkeyPatch,
):
...
def test_drop_oldest_under_queue_pressure(
    ...
    monkeypatch.setattr(publisher_module, "_QUEUE_MAXSIZE", 4)
    monkeypatch.setattr(publisher_module, "_publisher", None)
    instance = publisher_module._StreamPublisher()
    try:
