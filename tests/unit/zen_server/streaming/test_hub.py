...
def test_terminate_after_catchup_yields_end_marker():
    """Terminate after catchup yields end marker."""
    ...
    async def scenario():
        ...
        try:
            ...
            agen = hub.attach("k", terminate_after_catchup=True).__aiter__()
            first = await agen.__anext__()
            assert isinstance(first, BrokerEvent)
            second = await agen.__anext__()
            assert isinstance(second, EndMarker)
...
def test_terminate_after_catchup_on_empty_stream_emits_end_only():
    """Terminate after catchup on empty stream emits end only."""
    ...
    async def scenario():
        ...
        try:
            ...
            assert isinstance(first, EndMarker)
            with pytest.raises(StopAsyncIteration):
                await agen.__anext__()
        finally:
            await hub.shutdown()
...
def test_terminate_after_catchup_isolated_to_one_consumer():
    """The EndMarker must NOT terminate other consumers on the same stream."""

    async def scenario():
        broker = InMemoryBroker(max_len=10)
        hub = StreamHub(broker=broker, idle_grace_seconds=0.5)
        try:
            await broker.publish("k", [b"a"])

            persistent = hub.attach("k").__aiter__()
            # Drain initial catch-up so persistent is in live mode.
            assert (await persistent.__anext__()).payload == b"a"

            terminating = hub.attach(
                "k", terminate_after_catchup=True
            ).__aiter__()
            # First item is the historical event, then the synthetic end.
            assert (await terminating.__anext__()).payload == b"a"
            assert isinstance(await terminating.__anext__(), EndMarker)

            # Persistent consumer must still be alive and able to
            # receive new events.
            await broker.publish("k", [b"b"])
            live_item = await asyncio.wait_for(
                persistent.__anext__(), timeout=2.0
            )
            assert live_item.payload == b"b"
            await persistent.aclose()
            await terminating.aclose()
        finally:
            await hub.shutdown()
...
def test_initial_cursor_failure_aborts_reader_instead_of_replaying():
    ...
    async def scenario():
        ...
        hub = StreamHub(broker=broker, idle_grace_seconds=0.1)
        # Patch reconnect window so the test runs fast.
        import zenml.zen_server.streaming.hub as hub_mod

        hub_mod._INITIAL_CURSOR_RETRIES = 2
        hub_mod._RECONNECT_BACKOFF_INITIAL = 0.01
        try:
            agen = hub.attach("k").__aiter__()
